...
{   // Declare and initialize variables
    ...
    for(i = 0; i < pInfoHeader->Height; i++) { 
        ...
        for(j = 0; j < pInfoHeader->Width; j++) {
            ...
            if (is_skin(pImage[i][j].Red,pImage[i][j].Green,pImage[i][j].Blue)) {
                if (NON_BACKGROUND == cur_state) {
                    ...
                    if (cur_interval_size > interval_thresh) {
                        if (interval_list.size() > 0 && interval_list.back().first == NON_BACKGROUND) {
                            cur_interval_size += interval_list.back().second;
                            interval_list.pop_back();
                        }
                        pair<int, int> cur_interval (NON_BACKGROUND, cur_interval_size);
                        interval_list.push_back(cur_interval);
                ...
                if (BACKGROUND == cur_state) {
                    ...
                    if (interval_list.size() > 0 && cur_interval_size > interval_thresh) {
                        pair<int, int> cur_interval (BACKGROUND, cur_interval_size);
                        if (interval_list.back().first == BACKGROUND) {
                            cur_interval_size += interval_list.back().second;
                            interval_list.pop_back();
                        }
                        interval_list.push_back(cur_interval);
            ...
            if (j == pInfoHeader->Width - 20) {
                // decision
                for (vector< pair<int, int> >::iterator it = interval_list.begin() ; it != interval_list.end(); ++it) {
                    if (it->first == NON_BACKGROUND) {
                        skin_interval += 1;
                    }
                }
            }
        ...
        //     cout << "    skin_interval=" << skin_interval;
        //     cout << "\n";
        // }
